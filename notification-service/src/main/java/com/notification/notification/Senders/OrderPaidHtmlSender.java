...
@Component
public class OrderPaidHtmlSender implements HtmlSender<OrderEvent> {

    private final JavaMailSender mailSender;
    private final EmailLoader emailLoader;

    public OrderPaidHtmlSender(JavaMailSender mailSender, EmailLoader emailLoader) {
    ...
    public void send(String to, OrderEvent response) {
        ...
        try {
            ...
            String htmlBody = emailLoader.loadTemplate("order-paid.html");

            String mainHtml = htmlBody
                    .replace("${orderNumber}", response.getOrderNumber())
                    .replace("${paidAt}",      response.getCreatedAt().toString())
            ...
            helper.setSubject("Заказ №" + response.getOrderNumber() + " успешно оплачен");
